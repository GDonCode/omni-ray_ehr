...
export default function AppointmentCalendar({ onSelectSlot, selectedDate, selectedTime }: AppointmentCalendarProps){
    ...
    <div className='flex gap-4 overflow-x-auto max-w-full lg:overflow-visible lg:grid lg:grid-cols-3 lg:gap-6 pb-2'>
        {filteredDays.map((day, index) => {
            ...
            return (
                <div key={index} className='flex flex-col min-w-[200px] lg:min-w-0 lg:flex-row lg:w-full rounded-md border-2 border-white/20 bg-white/5 backdrop-blur-sm overflow-hidden hover:border-white/30 transition-all flex-shrink-0'>
                    ...
                    <div className='text-center p-4 flex flex-col lg:flex-wrap lg:flex-row lg:justify-center gap-4'>
                        {slots.map((slot, idx) => (
                            <button key={idx} onClick={() => handleSlotClick(dayKey, slot)} className={`border-2 border-white/20 py-4 px-6 rounded-md font-bold cursor-pointer select-none transition-all duration-300 hover:scale-105 text-[#036d6d] text-lg ${selectedSlot.dayKey === dayKey && selectedSlot.slot === slot ? 'border-none shadow-lg ring-4 ring-[#FFD700] bg-white' : 'bg-white'}`}>
                                {slot}
                            </button>
                        ))}
                    </div>
